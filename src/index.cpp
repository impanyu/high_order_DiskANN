    ...
    for (int i = 0; i < medoids.size(); i++){
        ...
        for (auto iter = clusters[i].begin();  iter != clusters[i].end(); ++iter){
            ...
            for (auto iter2 = clusters[i].begin();  iter2 != clusters[i].end(); ++iter2){
                float current_d = _data_store->get_distance(*iter, *iter2)/(_data_store->get_distance(*iter2,location)+1e-6);
                d += current_d;
                m_d = std::max(current_d,m_d);
            }
            //d = d/clusters[i].size();
            if (d < min_d){
                min_d = d;
    ...
    do{
        ...
        for (int i=0;i<pool.size();i++){
            if(i==0){
                ...
                float min_d =  std::numeric_limits<float>::max();
                float second_min_d = std::numeric_limits<float>::max();
                int min_id = 0;
                float min_r = 0;
                ...
                //if (second_min_d >= cur_alpha2*min_d || pool[i].distance >= cur_alpha*min_d){
                 if (_data_store->get_distance(medoids[min_id],location)>pool[i].distance*cur_alpha2 && pool[i].distance >= cur_alpha*min_d){
