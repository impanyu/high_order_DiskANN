                                          ...
                                          const tsl::robin_set<uint32_t> *const delete_set_ptr)
{
   ...
   for (int i = 0; i<pool.size(); i++){
       cur_result.push_back(pool[i].id);
   }
   int final_size = std::min(alphas_length,int(degree));
   final_size = std::min(final_size, int(pool.size()));
   cur_result.resize(final_size);
   return;
   ...
   /* for(;cur_alpha>=1; cur_alpha = cur_alpha - 0.1){
         if (cur_alpha >= _indexingAlphas[0]-(1e-6))
              ...
              cur_alpha2 = 1.2;*/
    for(; cur_alpha >=0.8; cur_alpha = cur_alpha -0.1){
    ...
    while(MST.size() < pool.size()+1){
            ...
            for (auto iter = pool.begin();  iter != pool.end(); ++iter){
                ...
                float c =   alphas[layer[min_id]] * (1+( std::pow(ratio,non_linear)*2-1)*slope);
